...
namespace tinyredis
{
    ...
    {
        handlers_.emplace("PING", [this](DataStore& store, const RespArray& args) {
            return handle_ping(store, args);
        });
        handlers_.emplace("ECHO", [this](DataStore& store, const RespArray& args) {
            return handle_echo(store, args);
        });
        handlers_.emplace("SET", [this](DataStore& store, const RespArray& args) {
            return handle_set(store, args);
        });
        handlers_.emplace("GET", [this](DataStore& store, const RespArray& args) {
            return handle_get(store, args);
        });
        handlers_.emplace("DEL", [this](DataStore& store, const RespArray& args) {
            return handle_del(store, args);
        });
